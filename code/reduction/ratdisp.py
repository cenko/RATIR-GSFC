...
def ratdisp( working_dir='.', cams=[0,1,2,3] ):
	...
	for d in target_dirs:
		...
		# make object and sky directories if they do not exist
		if not os.path.exists( af.OBJ_NAME ): os.makedirs( af.OBJ_NAME )
		if not os.path.exists( af.SKY_NAME ): os.makedirs( af.SKY_NAME )
		...
		for cam_i in cams:
			...
			if cam_i in [0,1]:
				...
				fout_img = open( '{}/{}_C{}.list'.format( af.OBJ_NAME, af.OBJ_NAME, cam_i ), 'w' ) # create list file for new FITs files
				fout_sky = open( '{}/{}_C{}.list'.format( af.SKY_NAME, af.SKY_NAME, cam_i ), 'w' ) # create list file for new FITs files
				...
				for line in fin:
					...
					while not valid_entry:
						...
						if direction.lower() == 'y':
							...
							# object frame
							imfits = '{}/{}_{}_{}.fits'.format( af.OBJ_NAME, fits_id, af.OBJ_NAME, cam_i )
							h['TARGNAME'] = targname
							hdulist.writeto( imfits, clobber=True ) # save object frame
							fout_img.write( fits_id + '\n' ) # write new file name to list
							
							# sky frame
							skyfits = '{}/{}_{}_{}.fits'.format( af.SKY_NAME, fits_id, af.SKY_NAME, cam_i )
			...
			if cam_i in [2,3]:
				fin = open( '{}_{}.list'.format( af.CAM_NAMES[cam_i], d ), 'r' ) # open list of FITs files
				fout_img = [open( '{}/{}_{}.list'.format( af.OBJ_NAME, af.OBJ_NAME, af.H2RG_FILTERS[cam_i-2] ), 'w' ), open( '{}/{}_{}.list'.format( af.OBJ_NAME, af.OBJ_NAME, af.H2RG_FILTERS[cam_i] ), 'w' )] # create list files for new img FITs files (e, w)
				fout_sky = [open( '{}/{}_{}.list'.format( af.SKY_NAME, af.SKY_NAME, af.H2RG_FILTERS[cam_i-2] ), 'w' ), open( '{}/{}_{}.list'.format( af.SKY_NAME, af.SKY_NAME, af.H2RG_FILTERS[cam_i] ), 'w' )] # create list files for new sky FITs files (e, w)
				...
				for line in fin:
					...
					while not valid_entry:
						...
						if direction.lower() == 'e' or direction.lower() == 'w': # selected
							...
							imfits = '{}/{}_{}_{}.fits'.format( af.OBJ_NAME, fits_id, af.OBJ_NAME, af.H2RG_FILTERS[f_img] )
							...
							skyfits = '{}/{}_{}_{}.fits'.format( af.SKY_NAME, fits_id, af.SKY_NAME, af.H2RG_FILTERS[f_sky] )
