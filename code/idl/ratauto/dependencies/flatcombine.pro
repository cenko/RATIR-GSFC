...
if n_elements(mincounts) eq 0 then mincounts = 30
...
for f = 0, nfiles-1 do begin
   ...
   if inmed ge mincounts and inmed le maxcounts then begin
      if keyword_set(indresp) then begin
         iresp = median(indata, dimension=2) ; if each flat has a different lamp spectrum (was heating up, etc.)...
   ...
   for y = 0, ny-1 do begin    
     for x = 0, nx-1 do begin  ; x is the fast array and should be iterated inside
       ...
       if i mod 157557 eq 0 then begin
          st = stddev(data[x, y, *])
          val = flat[x, y]
          if keyword_set(verbose) then print, clip(x,5), clip(y,5), 'stdev=',fpr(st,1.4), '  val=',fpr(val,1.4);, '  stdev/sqrt(val)=',fpr(st/sqrt(val),3.2) 
       endif
     endfor
   endfor
endif
...
sxaddpar, h, 'BZERO', 0
for f = 0, z-1 do begin
  sxaddpar, h, 'FLAT'+strtrim(string(f),2), removepath(usefiles[f])
endfor

if keyword_set(type) then sxaddpar,  h, 'FLATTYPE', type
