...
;       Modified version adds some extra keywords.
;
; CALLING SEQUENCE: 
...
;       2012-Jan-02     DAP             - added extra keywords
;-
;------------------------------------------------------------------------
...
;------------------------------------------------------------------------
function readmhdufitsdap, filename, HEADER=header, $
...
sxaddpar, header, 'NAMP', n_ext, 'number of active amps'
ampnum = intarr(n_ext+1) ; skips zero
for i = 1, n_ext do ampnum[i] = repstr(extname[i],'VidInp','')
...
if n_elements(gain) gt 0 then gaindata = build_gaindata(gain)
...
for i=1,n_ext do begin
    ...
    if keyword_set(NOTRIM) then begin
        ...
        array[xs:xe,*] = predata
        sxaddpar, header, 'PREPIX'+clip(ampnum[i]), clip(xs)+','+clip(xe)
        ...
        array[xs:xe,*] = data
        sxaddpar, header, 'AMP'+clip(ampnum[i]), clip(xs)+','+clip(xe)
        ;sxaddpar, header, 'COUNTS'+clip(ampnum[i]), median(data)
        ...
        sxaddpar, header, 'POSTPIX'+clip(i), clip(xs)+','+clip(xe)
        ...
        sxaddpar, header, 'AMP'+clip(ampnum[i]), clip(xs)+','+clip(xe)
        ;sxaddpar, header, 'COUNTS'+clip(ampnum[i]), median(data[*,yin1:yin2])
