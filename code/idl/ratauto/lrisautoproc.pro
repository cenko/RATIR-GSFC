...
pro autolrisdefaults
   ...
   for p = -1, 1 do begin
      ...
      if n gt 0 then begin
         ...
         endwhile
;         print 'Using parameters in ', sfile
...
; -------------------------
;pro autolrisrename
;   common lrisauto, autoastrocommand, swarpcommand, datadir, imworkingdir, spworkingdir, imfinaldir, spfinaldir, lrisversion, wildchar, overwrite, fileseq, filt;erstr, gratingstr, targetstr
;
;   redfiles = [findfile(datadir+'lred*.fits'),findfile(datadir+'rfoc*.fits')]
;   for f = 0, n_elements(redfiles)-1 do begin
;      file = redfiles[f]
;      if file eq '' then continue
;      h = headfits(file)
;      ut = sxpar(h, 'DATE', /silent)
;      filenum = strmid(file,4,4)
;      newfile = 'r'+strmid(ut,2,2) + strmid(ut,5,2) + strmid(ut,8,2) + '_' + filenum + '.fits'
;      command = 'mv ' + file + ' ' + datadir+newfile
;      spawn, command
;   endfor
;
;
;   bluefiles = [findfile('lblue*.fits'),findfile('bfoc*.fits')]
;   for f = 0, n_elements(bluefiles)-1 do begin
;      file = bluefiles[f]
;      if file eq '' then continue
;      h = headfits(file)
;      ut = sxpar(h, 'DATE', /silent)
;      filenum = strmid(file,strpos(file,'.')-4,4)
;      newfile = 'b'+strmid(ut,2,2) + strmid(ut,5,2) + strmid(ut,8,2) + '_' + filenum + '.fits'
;      command = 'mv ' + file + ' ' + datadir+newfile
;      spawn, command
;   endfor
;
...
;  prefchar = strmid(camstr,0,1) ;old prefix for LRIS meaning Red or Blue camera.  But RATIR has only 1, so we use prefchar = 2 (i.e., 2012...)
  prefchar = '2'
  ...
  ;'~/research/redux/allgrbpos.txt' 
  ;'~/research/redux/standardpos.txt'
...
;        pane = strtrim(sxpar(h, 'PANE'),2)
;        window = strtrim(sxpar(h, 'WINDOW'),2)
;        slit =  strlowcase(clip(sxpar(h, 'SLITNAME', count=cts)))
;        filemode = ''
;        if slit eq 'direct' then filemode = 'i'
;        if strpos(slit,'_') gt 0 then filemode = 's'
;        if filemode ne 'i' and filemode ne 's' then begin
;           if cts eq 0 then print, files[f], ': SLITNAME keyword absent; cannot distinguish image/spectrum'
;           print, files[f], ': Unrecognized slit keyword value SLITNAME=', slit, ' cannot distinguish image/spectrum'
;           print, '   Skipping...'
;           continue
;        endif
;
;        if mode eq 'i' and filemode ne 'i' then continue
;        if mode eq 's' and filemode ne 's' then continue
;        if filemode eq 'i' then outname = outnameim
;        if filemode eq 's' then outname = outnamesp
        outname=outnameim
        
        ;print, 'Preparing ', files[f]
print,outname
...
pro autolrismakeimflat, chip=chip, camera=camera
   ...
   common lrisauto, autoastrocommand, swarpcommand, datadir, imworkingdir, spworkingdir, imfinaldir, spfinaldir, lrisversion, wildchar, overwrite, fileseq, filterstr, gratingstr, targetstr
    ...
    for f = 0, n_elements(files)-1 do begin
       ...
       ; print, files[f], ' ', clip(targets[f],12), string(exps[f],format='(I4)'), ' ', clip(filters[f],5), ' ', clip(dichs[f],5), ' ', binnings[f], ' ', counts[f], domerelaz, isdomeflat[f], isskyflat[f], isscience[f]
       ;printzxx, files[f], counts[f], isdomeflat[f], isskyflat[f], isscience[f], elevation, domeazimuth, azimuth, domerelaz
    ...
    for i = 0, n_elements(scifilterlist)-1 do begin
      ...
      for d = 0, n_elements(dichlist)-1 do begin
        ...
        for b = 0, n_elements(binlist)-1 do begin
          ...
          ;print, filt, ' ', dich, ' ', bin
          ;for f = 0, n_elements(filtskyflatlist)-1 do print, '   ', filtskyflatlist[f], ' (', strtrim(string(counts[(where(files eq filtskyflatlist[f])) [0]]),2), ' counts)'
           ...
           ; First, see if we have enough frames to do a supersky flat.
            ...
            if ctposs gt 0 then begin ;otherwise usually means some weird situation like the dichroic info is missing, etc.
               ...
               if file_test('badflattargets.txt') eq 0 then begin
                  ...
                  ;badflattargets = grabcolumn('badflattargets.txt', 0, /str)
               ...
               ;print, ctsupersky, nsuperskyfields
               ;if (ctsupersky ge  9 and nsuperskyfields ge 5) or (ctsupersky ge 17 and nsuperskyfields ge 4) or $
               ;   (ctsupersky gt 17 and nsuperskyfields gt 3) or  ctsupersky ge 21 then begin
...
;--------------------------
;
pro autolrismakespecflat, chip=chip, camera=camera
;
;   common lrisauto, autoastrocommand, swarpcommand, datadir, imworkingdir, spworkingdir, imfinaldir, spfinaldir, lrisversion, wildchar, overwrite, fileseq, filterstr, gratingstr, targetstr
;
...
;    prefchar = strmid(camera,0,1)
;
;    files = findfile(spworkingdir+'p'+prefchar+wildchar+fchip+'.fits')
;
;    isdomeflat = intarr(n_elements(files))
;    ishalogen  = intarr(n_elements(files))
;    isskyflat  = intarr(n_elements(files))
;    isscience  = intarr(n_elements(files)) 
;    gratings =  strarr(n_elements(files))  
;    dichs =  strarr(n_elements(files))  
;    binnings = strarr(n_elements(files))
;    wins =  strarr(n_elements(files))  
;    filters = strarr(n_elements(files))
...
;
   for f = 0, n_elements(files)-1 do begin
       ...
       filters[f] = clip(sxpar(h, 'FILTER'))
       counts[f] =  sxpar(h,'COUNTS')
       if prefchar eq 'b' then gratkey = 'GRISNAME'
       if prefchar eq 'r' then gratkey = 'GRANAME'
       gratings[f] = clip(sxpar(h,gratkey))
       dichs[f] = clip(sxpar(h,'DICHNAME'))
       binnings[f] = repstr(clip(sxpar(h,'BINNING')),',','x')
       wins[f] = clip(sxpar(h,'PANE'))
       if wins[f] eq '0' then wins[f] = clip(sxpar(h,'WINDOW'))
       targets[f] = clip(sxpar(h,'TARGNAME'))
       trapdoor = clip(sxpar(h,'TRAPDOOR'))
       lamps = clip(sxpar(h,'LAMPS'))
       ...
       while domerelaz lt -180.0 do domerelaz += 360.
       while domerelaz ge  180.0 do domerelaz -= 360.
       if trapdoor eq 'open' then begin 
         if (abs(elevation-45) lt 0.5 and abs(domerelaz) gt 3) or strpos(targets[f],'cass') ge 0 then begin
             isdomeflat[f] = 1  ;if counts[f] gt 7000 and counts[f] lt 40000 then 
         endif else begin
             if counts[f] gt 10000 and counts[f] lt 43000 and exps[f] lt 300 then isskyflat[f] = 1
             if abs(domerelaz) lt 5.0 and (counts[f] lt 1000) then isscience[f] = 1
         endelse
       endif

       if counts[f] gt 500 and counts[f] lt 43000 and trapdoor eq 'closed' and strmid(lamps,10,1) eq '1' then ishalogen[f] = 1

       ;print, files[f], ' ', clip(targets[f],12), string(exps[f],format='(I4)'), ' ', clip(gratings[f],9), ' ', counts[f], domerelaz, ' ', clip(trapdoor,6), ' ', lamps, ' ', clip(isdomeflat[f]), ' ', clip(ishalogen[f])
  
   endfor

   sciindex = where(isscience, ct)
   if ct eq 0 then return
   scigratinglist  = unique(gratings[sciindex])
   for i = 0, n_elements(scigratinglist)-1 do begin
      grat = scigratinglist[i]
      gratstr = repstr(grat,'/','-')
      dichlist = unique(dichs[sciindex[where(grat eq gratings[sciindex])]])
      for d = 0, n_elements(dichlist)-1 do begin
        dich = dichlist[d]
        if n_elements(dichlist) gt 1 then dichstr = 'D'+dichlist[d] else dichstr = ''
        binlist = unique(binnings[sciindex[where(grat eq gratings[sciindex] and dich eq dichs[sciindex])]])
        for b = 0, n_elements(binlist)-1 do begin
          bin = binlist[b]
          if n_elements(binlist) gt 1 then binstr = 'b'+binlist[b] else binstr = ''
          outflatname = spworkingdir+'lris'+strmid(camera,0,1)+'flat'+gratstr+dichstr+binstr+'.fits'
          ;print, 'Need ', outflatname
          if file_test(outflatname) and overwrite eq 0 then continue ; flat exists already
          flatsuccess = 0

          domeflats = where(isdomeflat and gratings eq grat and dichs eq dich and binnings eq bin and counts gt 1000. and counts lt 39000., ctdome)
          dwindows = wins[domeflats]
          winlist = unique(dwindows, count=nwin)
          mostcommonwin = winlist[(where(nwin eq max(nwin))) [0]] ; what if this is different from science windowing...?
          domeflats = where(isdomeflat and gratings eq grat and dichs eq dich and binnings eq bin and counts gt 1000. and counts lt 39000. and wins eq mostcommonwin, ctdome)
          if ctdome lt 3 then $
             domeflats = where(isdomeflat and gratings eq grat and dichs eq dich and binnings eq bin and counts gt 100. and counts lt 39000. and wins eq mostcommonwin, ctdome)



          ; THING TO CHANGE IN THE FUTURE - this counts estimate, the the hal one below, is not appropriate for the lowest-res blue grating. - fixed now?
          ;print, grat, ' ', dich,' ', bin, ' : ' ctdome
          if ctdome ge 3 then begin
             print, grat, ', D'+dich,': ', strtrim(string(ctdome),2), ' dome flats'
             h = headfits(files[domeflats[0]])
             yresp0 = fix((strsplit(sxpar(h,'AMPR1'),',',/extract)) [0]) + 10
             yresp1 = yresp0 + 260
             ;if lrisversion eq 2 and camera eq 'red' then begin
             ;   yresp = findgen(580)  ; eeeew hard coded
             ;   yresp = yresp[where((yresp lt 182) or (yresp gt 256 and yresp lt 404) or (yresp gt 472 and yresp lt 577))]
             ;   flatcombine, files[domeflats], outflatname, type='dome', /response, yresp=yresp
             ;endif else 
            if camera eq 'blue' then begin
               ; the blue side has some lines that a median response algorithm will miss 
               ; finding and distinguishing from bad columns is tough, so just tell it where to search
               ; needs to be updated for binned world and different gratings...
               flatcombine, files[domeflats], outflatname, type='dome', /response;;, lines=1399, linewidth=56  ; [1383,1412]
            endif else begin
               flatcombine, files[domeflats], outflatname, type='dome', /response
            endelse 
            flatsuccess = 1
          endif
          if flatsuccess eq 0 then begin
            halflats = where(ishalogen and gratings eq grat and dichs eq dich and counts gt 500. and counts lt 39000., cthal)
            if cthal ge 2 then begin
               print, grat, ', D'+dich, ' ', bin, ': ', strtrim(string(cthal),2), ' halogen flats'
               print, 'WARNING - Halogen flats tend to be poor, next time get spectroscopic dome flats...'
               flatcombine, files[halflats], outflatname, type='halogen', /response, /indresp, /slitnorm
               flatsuccess = 1
            endif
          endif
          if flatsuccess eq 0 then begin
            print, 'Unable to produce a flat field for this combination: ', grat, ' & ', 'D'+dich+' ('+ bin + ').'
            noproc = where(isscience and gratings eq grat and dichs eq dich, ctnoproc)
            print, 'Will not be able to further process ', clip(ctnoproc), ' images without a flat from another source:'
            for j = 0, ctnoproc-1 do print, '   ', files[noproc[j]], exps[noproc[j]], '  ', targets[noproc[j]]
          endif

        endfor ; binning loop
      endfor   ; dichroic loop
    endfor     ; grating loop
...
;       inst = strtrim(sxpar(h, 'INSTRUME'),2)
;       if inst eq 'LRIS' then instabbrev = 'r'
;       if inst eq 'LRISBLUE' then instabbrev = 'b'
;       if instabbrev ne strmid(camera,0,1) then continue
       filter = clip(sxpar(h, 'FILTER'))
;stop
;       dich = clip(sxpar(h, 'DICHNAME'))
;       win = clip(sxpar(h,'WINDOW'))
;       if clip(sxpar(h,'PANE')) ne '0' then win = clip(sxpar(h,'PANE')) else win = strmid(win,2) ; if available, more useful
;       flatchip = clip(sxpar(h, 'CHIP'))
       flatfilts[f] = filter
;       flatdichs[f] = dich
;       flatwins[f] = win ; some unused characters at the start, or something
;       flatbins[f] = clip((sxpar(h,'BINNING')))
;       flatchips[f] = flatchip
     endfor
   endif else begin
     nopflats = 1
   ...
   endelse
    ...
    for f = 0, n_elements(files)-1 do begin
      ...
      if ct eq 0 or overwrite then begin               
         h = headfits(files[f], /silent)
         ;;fits = mrdfits(files[f], 0, h, /silent)
...
;         while domerelaz lt -180.0 do domerelaz += 360.
;         while domerelaz ge  180.0 do domerelaz -= 360.
;         ;print, files[f], elevation, domerelaz, target, counts, exptime, counts/exptime
;                                         ; (abs(domerelaz-90) lt 15 or abs(domerelaz+90) lt 15) 
;         if abs(elevation-45) lt 0.5 and abs(domerelaz) gt 3 then continue ; silently skip dome flats
;         if strpos(target,'cass') ge 0 then continue 
;         if strpos(target,'dome') ge 0 then continue ; sometimes the telescope is pointed wrong; we still don't want these...
;         if strpos(target,'horizon') ge 0 then continue 
;
;         if counts gt 40000 then continue  ; skip anything close to saturation
;         if counts gt 30000. and exptime lt 10 then continue
;         if counts/exptime gt 10000. and exptime gt 1 then begin
;            print, files[f], ' is probably a sky flat: skipping'
;            continue
;         endif
         filter = clip(sxpar(h, 'FILTER'))
;         dich = clip(sxpar(h, 'DICHNAME'))
;         win = strmid(clip(sxpar(h,'WINDOW')),2)
;         if clip(sxpar(h,'PANE')) ne '0' then win = clip(sxpar(h,'PANE')) else win = strmid(win,2)
;         binning = clip((sxpar(h,'BINNING')))
         binning='1'
;         flatchip = clip(sxpar(h, 'CHIP'))
;         flatfileno = where(flatfilts eq filter and flatchips eq flatchip and flatdichs eq dich and flatbins eq binning, ct) ; and flatwins eq win
         flatfileno = where(flatfilts eq filter, ct) ;all we care about is the filter for RATIR

;         ;print, flatfilts, '(', filter,')'
;         ;print, flatchips,'(',flatchip ,')'
;         ;print, flatdichs, '(', dich,')' 
;         ;print, flatwins, '(', win,')'
;
         if ct eq 0 then begin
;            if nopflat eq 1 then begin
;               print, 'No processed flats found!  Cannot perform flatfielding.'
;               return
;            endif
;            flatfilenoothbin = where(flatfilts eq filter and flatchips eq flatchip and flatdichs eq dich, ctwobin) ; and flatwins eq win
         ...
         flatproc, files[f], flatfile, flatminval=0.7, crop='auto'  ; add autocropping of low-signal zones

;         ;if flatchip eq 'right' then flatproc, files[f], flatfile, crop=rcrop
;         ;if flatchip eq 'left' then flatproc, files[f], flatfile, crop=lcrop
;         ;if flatchip eq 'old' then flatproc, files[f], flatfile, crop=ocrop
;         ;if flatchip eq 'old' and keyword_set(noxfix) eq 0 then lrisxfix, 'f'+files[f], prefix=''
;         ;if flatchip ne 'right' and flatchip ne 'left' and flatchip ne 'old' then print, 'Chip not recognized - cannot flatten'
      endif
    endfor
...
;       win = clip(sxpar(h,'WINDOW'))
;       if clip(sxpar(h,'PANE')) ne '0' then win = clip(sxpar(h,'PANE')) else win = strmid(win,2) ; if available, more useful
;       skychip = clip(sxpar(h, 'CHIP'))
       skyfilts[f] = filter
;       skydichs[f] = dich
;       skywins[f] = win ; some unused characters at the start, or something
;       skybins[f] = clip((sxpar(h,'BINNING')))
;       skychips[f] = skychip
     endfor
   endif else begin
     nopskys = 1
...
; -------------
;
;pro autolrisspecflatten, chip=chip, camera=camera
;
;   common lrisauto, autoastrocommand, swarpcommand, datadir, imworkingdir, spworkingdir, imfinaldir, spfinaldir, lrisversion, wildchar, overwrite, fileseq, filterstr, gratingstr, targetstr
;   common lrisfail, flatfail, catastrofail, relastrofail, fullastrofail, extractfail, wavsolfail, wavsolwarn, fluxcalfail
;
...
;  pref2char = ''
;
;  files = findfile(spworkingdir+'p'+pref2char+prefchar+wildchar+'.fits')
;  ffiles = findfile(spworkingdir+'fp'+pref2char+prefchar+wildchar+'.fits')
;
;  if n_elements(files) eq 1 and files[0] eq '' then return
;
;  ; Search for existing flatfields
;  noflats = 0
;  ycrop = [1,1]
;
;  camchar = strmid(camera,0,1)
;  if camchar eq 'b' then instname = 'LRISBLUE'
;  if camchar eq 'r' then instname = 'LRIS'
;  if camchar eq 'b' then gratkey = 'GRISNAME'
;  if camchar eq 'r' then gratkey = 'GRANAME'
;
;  flats = findfile(spworkingdir+'*flat*.fits')
;
;  flatgrats = strarr(n_elements(flats))
;  flatchips = strarr(n_elements(flats))
;  flatdichs = strarr(n_elements(flats))
;  ;flatwins = strarr(n_elements(flats))
;  flatbinnings = strarr(n_elements(flats))
;  flatycrop = intarr(n_elements(flats),2)
;  if flats[0] ne '' then begin
;     for f = 0, n_elements(flats)-1 do begin
;       h = headfits(flats[f], /silent)
;       inst = strtrim(sxpar(h, 'INSTRUME'),2)
;       if inst ne instname then continue
;       grating = clip(sxpar(h, gratkey))
;       dich = clip(sxpar(h, 'DICHNAME'))
;       win = clip(sxpar(h,'WINDOW'))
;       if clip(sxpar(h,'PANE')) ne '0' then win = clip(sxpar(h,'PANE')) else win = strmid(win,2)
;       flatchip = clip(sxpar(h, 'CHIP'))
;       flatgrats[f] = grating
;       flatdichs[f] = dich
;       flatbinnings[f] = repstr(clip(sxpar(h,'BINNING')),',','x')
;       flatchips[f] = flatchip
;       if total(ycrop eq [0,0]) eq 2.0 then begin  ; automatic cropping
;          flat = mrdfits(flats[f], /silent)
;          s = size(flat)
;          nx = s[1]
;          yprofile = total(flat[0:nx/5.,*], 1)/median(flat[0:nx/5.,*]) + $
;                     total(flat[4*nx/5.:nx-1,*], 1)/median(flat[4*nx/5.:nx-1,*])
;                     ; sum over x-axis, using the edge areas since we are liberally cropping out all rows with some
;                     ; signal-less pixels and the spectrum is often tilted
;          signalzone = where(yprofile gt 0.86*median(yprofile)) ; hopefully there's no slit profile variation.
;          flatycrop[f,*] = [min(signalzone)+1,max(signalzone)-2]
;
;       endif
;     endfor
;  endif else begin
;     print, 'No flats found!  Cannot perform flatfielding.'
;     return
;  endelse
;   
;   ; ycrop = median(flatycrop, dimension=1) ; assume it's the same for all gratings (and slits)
...
;         ;;fits = mrdfits(files[f], 0, h, /silent)
;         counts = sxpar(h, 'SKYCTS') > sxpar(h,'COUNTS')
;         exptime = float(sxpar(h, 'ELAPTIME'))
;         azimuth = sxpar(h,'AZ')
;         elevation = sxpar(h,'EL')
;         domeazimuth = sxpar(h,'DOMEPOSN')
;         target = sxpar(h,'TARGNAME')
...
;
;         ; skip biases
;         if exptime lt 0.5 then continue
;
;         ; skip halogens
;         lampstr = sxpar(h,'LAMPS')
;         lamps = strsplit(lampstr,',',/extract)
;         if lamps[5] ne '0' then continue ; internal halogen on
;
;         domerelaz = domeazimuth - azimuth
;         while domerelaz lt -180.0 do domerelaz += 360.
;         while domerelaz ge  180.0 do domerelaz -= 360.
...
;
;         ; skip dome flats
;         if total(lamps[0:5] ne '0') eq 0 and clip(sxpar(h,'TRAPDOOR')) eq 'open' then begin
;           if abs(elevation-45) lt 0.5 and abs(domerelaz) gt 3 then continue ; silently skip dome flats
;           if strpos(target,'cass') ge 0 then continue 
;           if strpos(target,'dome') ge 0 then continue ; sometimes the telescope is pointed wrong; we still don't want these...
;           if strpos(target,'horizon') ge 0 then continue 
...
;         grating = clip(sxpar(h, gratkey))
;         dich = clip(sxpar(h, 'DICHNAME'))
;         binning = repstr(clip(sxpar(h,'BINNING')),',','x')
;         win = strmid(clip(sxpar(h,'WINDOW')),2)
;         if clip(sxpar(h,'PANE')) ne '0' then win = clip(sxpar(h,'PANE')) else win = strmid(win,2)
;
;         flatchip = clip(sxpar(h, 'CHIP'))
;         flatfileno = where(flatgrats eq grating and flatchips eq flatchip and flatdichs eq dich and flatbinnings eq binning, ct) ;and flatwins eq win
;         ;print, flatfilts, '(', filter,')'
;         ;print, flatchips,'(',flatchip ,')'
;         ;print, flatdichs, '(', dich,')' 
;         ;print, flatwins, '(', win,')'
;         ;print, flatbinnings, '(', binning, ')'
;
;         if ct eq 0 then begin
;            print, 'Flat field not found for ', removepath(files[f]), ' (', strtrim(grating,2), ', D', strtrim(dich,2), ', ', binning, ')'
;            flatfail = [flatfail, files[f]]
;            continue
;         endif
;         flatfile = flats[flatfileno[0]]
;         print, 'Flattening ', removepath(files[f]), ' using ', removepath(flatfile)
;         flatproc, files[f], flatfile, flatminval=0.1, crop='auto'
;                   ;crop=[0, ycrop[0], 0, ycrop[1]], 
;      endif
;    endfor
;
;
...
;; -------------------------
;pro autolrismakefringe
;
;   common lrisauto, autoastrocommand, swarpcommand, datadir, imworkingdir, spworkingdir, imfinaldir, spfinaldir, lrisversion, wildchar, overwrite, fileseq, filterstr, gratingstr, targetstr
;
;  if lrisversion eq 1 and makefringe ne 0 and camera eq 'red' then begin
;    ffiles = findfile('f'+prefchar+wildchar+'?'+'.fits')
;    fileinfo = replicate({filename:'', filter:'', counts:0., exptime:0., airmass:0., object:''}, n_elements(ffiles))
;    fileinfo.filename = ffiles
;    for f = 0, n_elements(fileinfo)-1 do begin
;      if fileinfo[f].filename eq '' then continue
;      h = headfits(fileinfo[f].filename, /silent)
;      fileinfo[f].filter = strtrim(sxpar(h, filtkey),2)
;      fileinfo[f].counts =  sxpar(h, 'COUNTS')
;      fileinfo[f].exptime = sxpar(h, 'ELAPTIME')
;      fileinfo[f].object  = sxpar(h, 'OBJECT')
;      fileinfo[f].airmass = sxpar(h, 'AIRMASS')
;      ;print, fileinfo[f].filename, ' ', fileinfo[f].filter, fileinfo[f].counts, fileinfo[f].exptime
;    endfor
;  
;    fringablefilts = ['I','RG850']
;    fringeexcludeobj = [''];['GRB070412', '050412', 'GRB051109B', 'GRB070521', 'GRB100205A']
;    for ff = 0, n_elements(fringablefilts)-1 do begin
;       ffilt = fringablefilts[ff]
;       fringefile = ffilt + 'fringe.fits'
;       if file_test(fringefile) then continue
;       thisfilt = where(fileinfo.filter eq ffilt, ct)
;       if ct lt 3 then continue
;       ffileinfo = fileinfo[thisfilt]
;       thisdark = where(ffileinfo.exptime gt 60 and ffileinfo.counts lt 40000, ct)
;       if ct lt 3 then continue
;       ffileinfo = ffileinfo[thisdark]
;       ctratethreshold = median((ffileinfo.counts/ffileinfo.exptime)/ffileinfo.airmass) * 1.8
;       thisdark = where((ffileinfo.counts/ffileinfo.exptime)/ffileinfo.airmass le ctratethreshold)
;       if ct lt 3 then continue
;       ffileinfo = ffileinfo[thisdark]
;       for nf = 0, n_elements(fringeexcludeobj)-1 do begin
;          notforbiddenobj = where(ffileinfo.object ne fringeexcludeobj[nf], ct)
;          if ct ge 1 then ffileinfo = ffileinfo[notforbiddenobj]
;       endfor
;       if ct lt 3 then continue
;
;       print, 'Making ', ffilt+'-band fringe frame...'
;
;       print, ffileinfo.filename
;
;       mkfringe, ffileinfo.filename, outfile=fringefile, /removeobjects
;    endfor
;  endif
;
;
...
;; -------------------------
;pro autolrisrmfringe
;
;   common lrisauto, autoastrocommand, swarpcommand, datadir, imworkingdir, spworkingdir, imfinaldir, spfinaldir, lrisversion, wildchar, overwrite, fileseq, filterstr, gratingstr, targetstr
;
;  if lrisversion eq 1 and rmfringe ne 0 and camera eq 'red' then begin
;    ffiles = findfile('f'+prefchar+wildchar+'?'+'.fits')
;    if overwrite eq 0 then ffiles = unmatched(ffiles,'i')
;    fileinfo = replicate({filename:'', filter:'', counts:0., exptime:0., object:''}, n_elements(ffiles))
;    fileinfo.filename = ffiles
;    for f = 0, n_elements(fileinfo)-1 do begin
;      if fileinfo[f].filename eq '' then continue
;      h = headfits(fileinfo[f].filename, /silent)
...
;       ffilt = fringablefilts[ff]
;       fringefile = ffilt + 'fringe.fits'
;       if file_test(fringefile) eq 0 then continue
;       thisfringe = where(fileinfo.filter eq ffilt and fileinfo.counts/fileinfo.exptime lt 1000., ctfringe)
;
;       if ctfringe ge 1 then begin
;         fringablefiles = fileinfo[thisfringe].filename
;         if fringablefiles[0] ne '' then $
;           fringecorrect, fringablefiles, fringefile=fringefile, prefix='i';, /iterate, itersec=[50,880,200,-200]
;         ; alternative method uses weights rather than iteration
;         ;fringescale = filesi.counts
;         ;continuumimg = where(filesi.counts/(filesi.exptime*filesi.airmass) gt 10, ctcontinuum)
;         ;if ctcontinuum gt 0 then fringescale[continuumimg] = (filesi[continuumimg].exptime*filesi[continuumimg].airmass)/0.15
;         ;fringecorrect, filesi.name, fringefile='Ifringe.fits', /iterate, itersec=[300,700,400,600]
;      endif
;    endfor
;  endif
;
...
;; -------------------------
;pro autolriscrcleanspec, chip=chip, camera=camera
;
;   common lrisauto, autoastrocommand, swarpcommand, datadir, imworkingdir, spworkingdir, imfinaldir, spfinaldir, lrisversion, wildchar, overwrite, fileseq, filterstr, gratingstr, targetstr
;
;camchar = strmid(camera,0,1)
;chipchar = strmid(chip,0,1)
;
;files = choosefiles(spworkingdir+'fp'+camchar+wildchar+chipchar+'.fits')
;
;if overwrite eq 0 then files = unmatched(files,'z')
;
;;if camchar eq 'b' then boxsize = 5
;;if camchar eq 'r' then boxsize = 7
;
;for f = 0, n_elements(files)-1 do begin
;   if files[f] eq '' then continue
;   if n_elements(fileseq) gt 0 then if total(filenum(files[f]) eq fileseq) eq 0 then continue
...
;   lamps = strsplit(lampstr,',',/extract)
;   ;;if total(lamps ne '0') gt 0 then continue ;  don't bother with lamps, arcs, etc.
;
;   trapdoor = clip(sxpar(h,'TRAPDOOR'))
;   if trapdoor eq 'closed' then continue
;
;   binning = clip(sxpar(h,'BINNING'))
;   bins = fix(strsplit(binning,',',/extract))
;
;   ;if (bins[0] ge 2 or bins[1] ge 2) and boxsize ge 5 then boxsize -= 2 
;
;   ; should use a faster and less dangerous algorithm for the blue side?
;
;   print, 'Cleaning cosmic rays from ', removepath(files[f])
;   pzapspec, files[f], usamp=(camchar eq 'b')
;
;endfor
;
;end
...
;; ------------------
;pro autolrissplit, chip=chip, camera=camera, mode=mode
;
;
;   common lrisauto, autoastrocommand, swarpcommand, datadir, imworkingdir, spworkingdir, imfinaldir, spfinaldir, lrisversion, wildchar, overwrite, fileseq, filterstr, gratingstr, targetstr
;
;camchar = strmid(camera,0,1)
;if n_elements(chip) gt 0 then chipchar = strmid(chip,0,1) else chipchar = ''
;
;if mode eq 'im' then workingdir = imworkingdir
;if mode eq 'sp' then workingdir = spworkingdir
;
;if chipchar eq 'r' then rightonly=1
;if chipchar eq 'l' then leftonly=1
;
;files = choosefiles(camchar+wildchar+'.fits',workingdir+'ifp',workingdir+'fp')
;
;for f = 0, n_elements(files)-1 do begin
;   if files[f] eq '' then continue
;   if n_elements(fileseq) gt 0 then if total(filenum(files[f]) eq fileseq) eq 0 then continue
;
;   rfile = repstr(files[f],'.fits','r.fits')
;   lfile = repstr(files[f],'.fits','l.fits')
;
;   if overwrite eq 0 then begin
;      if file_test(rfile) and file_test(lfile) then continue
;      if file_test(rfile) and keyword_set(rightonly) then continue
;      if file_test(lfile) and keyword_set(leftonly) then continue
;   endif
;
;
;   lrissplit, files[f], rightonly=rightonly, leftonly=leftonly
;
;endfor
;
;end
...
; -------------------------
pro autolrisskysubtract, camera=camera, chip=chip
...
files = choosefiles(camchar+wildchar+chipchar+'.fits',spworkingdir+'zfp',spworkingdir+'fp')
if overwrite eq 0 then files = unmatched(files,'s')
...
for f = 0, n_elements(files)-1 do begin
   ...
   lampstr = clip(sxpar(h,'LAMPS'))
   lamps = strsplit(lampstr,',',/extract)
   ;;if total(lamps ne '0') gt 0 then continue ;  skip flats/arcs

   trapdoor = clip(sxpar(h,'TRAPDOOR'))
   if trapdoor eq 'closed' then continue

   dum = subtractsky2(files[f])
...
pro autolrisextract, camera=camera, chip=chip
   ...
   for f = 0, n_elements(files)-1 do begin
      ...
      ;'~/research/Useful/skyspec/gident_most.dat'
      ...
      if ct eq 0 then begin
         ...
         continue
         ;print, 'Spectrum will not be wavelength calibrated.'
      endif else begin
         archere = 0
         ...
         if solexists eq 0 or archere then begin
            ...
            arclist = wav
                            ...
                            ;'~/research/redux/arc/lrisarcbright.list'
      ...
      if camchar eq 'r' then begin
         ;print, airmass, arcair
         ;if abs(airmass-arcair) le 0.01 then maxnudge = 10.
         ;if abs(airmass-arcair) gt 0.01 and abs(airmass-arcair) lt 1.0 then maxnudge = 100.
         ;if abs(airmass-arcair) ge 1.0 then maxnudge = 250.
         maxnudge = 220.
      endif
...
pro autolrisfluxcal, camera=camera, chip=chip
   ...
   for f = 0, n_elements(files)-1 do begin
      ...
      ;;targets[f] = standardizename(target, ismatch=ismatch) ; old method
      rastr = sxpar(h,'RA')
      decstr = sxpar(h,'DEC')
   ...
   for s = 0, nsci-1 do begin
      ...
      stdair = airmasses[wfluxcal[matchstandards]]
         ...
         ;for n2 = 3, 20 do print,( 911.760 * 1./(1./n1^2 - 1./n2^2) ) / (1.0 + 2.735182E-4 + 131.4182 / vac^2 + 2.76249E8 / vac^4)
...
pro autolriscombine, camera=camera
   ...
   for f = 0, n_elements(files)-1 do begin
     ...
     if strpos(target,'_S') gt 0 then begin  ; strip the offset star from the target info
        ...
        ;if ustriptarget ne 'HD' and ustriptarget ne 'BD' and ustriptarget ne 'PTF' then 
        target = ustriptarget
     endif
     targets[f] = target
...
;    prefchar = strmid(camera,0,1)
    ...
    ;   print, 'autoastrometry '+zffiles[f]+ ' -o' ; Initially, get a "rough" solution for every image using the catalog
    ;   spawn, 'autoastrometry '+zffiles[f]+ ' -o' ;  (for deep images to be used in stacking we will improve this later)
...
;          refcatfile = imworkingdir+targets[t]+'.cat'
      ...
      ;for i = 0, n_elements(imagesthistarg)-1 do begin
      ;   h = headfits(imagesthistarg[i])
      ;endfor
      ...
      ;pa = strtrim(string((float(sxpar(h, 'ROTPOSN'))+0) MOD 360),2)
          ...
          if file_test(outfile) eq 0 then begin
             ...
             print
          endelse
      ; ' -px '+strtrim(string(pxscale),2)+' -pa '+pa+               ;;+' -s 6'
...
;          refcatfile = imworkingdir + targname + '.'+camera+'.'+achip+'.cat'
...
;  if camera eq 'red' then camiter = clip(lrisversion) else camiter = ''
;
;  if camera eq 'blue' and file_test('zeropointsblue.dat') eq 0 then $              ; ichip eq 0 and 
;    lrisautostdphot, /blue ; chip-independent
;  if camera eq 'red' and countfiles('zeropointsred*.dat') eq 0 then $              ; ichip eq 0 and 
;    lrisautostdphot, /red
;  if file_test(imworkingdir+'autophotsummaryflux.'+camera+camiter+'.txt') eq 0 then $
...
;     fileexposures[f] = float(sxpar(h,'ELAPTIME'))
     fileexposures[f] = 60.
...
;        if max(fileexposures[thistargfilt]) lt 30. then continue
        expthresh = (median(fileexposures[thistargfilt]) * 0.5) > (max(fileexposures[thistargfilt]) * 0.19) ; formerly median*0.8
;        stacki = where(filetargets eq target and filefilters eq filter and fileexposures gt expthresh, ctstack)
        stacki = where(filetargets eq target and filefilters eq filter, ctstack)
        ;if ctstack lt 2 then continue
        if ctstack eq 0 then continue
        ;;if ctstack eq 1 then print, 'Only one deep exposure:  trivial stack'
...
;        outfile       = imworkingdir + 'coadd' + strtrim(target,2) +'_'+ strtrim(filter,2) + '.' + chipchar + '.fits'
;        outweightfile = imworkingdir + 'coadd' + strtrim(target,2) +'_'+ strtrim(filter,2) + '.' + chipchar + '.weight.fits'
        ...
        stackcmd = stackcmd + ' -COPY_KEYWORDS OBJECT,TARGNAME,TELESCOP,FILTER,DICHNAME,SLITNAME,'+$
                              ...
                              ;'DATE_BEG,DATE,END,UTC-END,DATE-END,BINNING,TRAPDOOR,LAMPS,FLIMAGIN,FLSPECTR'
          ; Too many keywords causes pyfits to be unable to load the mosaic.  It's rather freakish.
        ;stackcmd = stackcmd + ' -BACK_SIZE 256'
        ...
        if (file_test(outfile) eq 0) or overwrite then begin
           ...
           ;;print, autoastrocommand+' '+outfile+' -q'
           ;;spawn, autoastrocommand+' '+outfile+' -q'
           ;;                 ; do another round of precise astrometry on the stack (probably unnecessary now)
...
;     coaddfilesr = findfile(imworkingdir+'coadd*r.fits')
;     coaddfilesl = findfile(imworkingdir+'coadd*l.fits')
;     if coaddfilesr ne [''] then coaddfilesr = coaddfilesr[where(coaddfilesr ne '')] ;else continue
;     if coaddfilesl ne [''] then coaddfilesl = coaddfilesl[where(coaddfilesl ne '')] ;else continue
...
;        rfilename = coaddfilesr[f]
;        lfilename = repstr(rfilename, 'r.','l.')
        ;match = where(coaddfilesl eq lfilename, ct)
        filename = coaddfiles[f]
        if ct eq 1 then begin
;           stackcmd = swarpcommand + ' ' + rfilename + ',' + lfilename
;           outfile = repstr((coaddfilesr[f]), '.r.fits', '.fits')
           outfile = coaddfiles[f]
           if file_test(outfile) eq 0 or overwrite then begin
;              rweightname = repstr(rfilename, '.fits','.weight.fits') 
;              lweightname = repstr(lfilename, '.fits','.weight.fits') 
              weightname = repstr(filename, '.fits','.weight.fits') 
              ;if file_test(rweightname) eq 0 then spawn, 'mv ' + strmid(rweightname,1) + ' ' + rweightname
              ;if file_test(lweightname) eq 0 then spawn, 'mv ' + strmid(lweightname,1) + ' ' + lweightname
              print
;              print, 'Combining ', rfilename, ' and ', lfilename
     ...
     ;if crclean eq 0 then begin
     ;    azffiles = findfile('af'+prefchar+wildchar+'r.fits')
     ;endif else begin
     ;    azffiles = findfile('azf'+prefchar+wildchar+'r.fits')
     ;endelse
...
pro lrisautoproc, datadirectory=datadirectory, modestr=modestr, camerastr=camerastr, chipstr=chipstr, files=files, filters=filters, gratings=gratings, targets=targets, start=start, stop=stop, only=only, step=step, nocrclean=nocrclean, redo=redo, nofringe=nofringe, continuous=continuous
...
;if lastdir eq imworkingdir or lastdir eq imfinaldir or $
;   lastdir eq spworkingdir or lastdir eq spfinaldir or $ 
;   pwd eq imworkingdir or pwd eq imfinaldir or $
;   pwd eq spworkingdir or pwd eq spfinaldir  $ 
...
;if n_elements(camerastr) eq 0 then camerastr = 'rb'
;camerastr = strlowcase(camerastr)
;if camerastr eq 'br' or camerastr eq 'b,r' then cameras = ['blue', 'red']
;if camerastr eq 'rb' or camerastr eq 'r,b' then cameras = ['red', 'blue']
;if n_elements(cameras) eq 0 then begin
;   if strmid(camerastr,0,1) eq 'b' then cameras = ['blue']
;   if strmid(camerastr,0,1) eq 'r' then cameras = ['red']
;endif
;if n_elements(cameras) eq 0 then begin
;  print, 'Cannot recognize camera request: ', camerastr
;  return
;endif
...
;; Investigate the data and rename files to modern format if necessary.
;
;noldfiles = countfiles(datadir+'lred*.fits') + countfiles(datadir+'lblue*.fits')
; ;+ countfiles(datadir+'rfoc*.fits') + countfiles(datadir+'bfoc*.fits')
;if noldfiles gt 0 then begin
;   print, 'Image file names are in old format.  Updating...'
;   autolrisrename
;endif


;lrisversion = getlrisversion(datadir=datadir, /quiet)
;if lrisversion le 0 then lrisversion=getlrisversion(datadir=spworkingdir, /quiet)
;if lrisversion le 0 then lrisversion=getlrisversion(datadir=imworkingdir)
lrisversion = 3
...
;            if instep eq 'makeflat' then autolrismakeimflat,cam=camera, chip=''
...
;            if instep eq 'makeflat' then autolrismakespecflat, cam=camera, chip=''
;            if instep eq 'flatten'  then autolrisspecflatten,  cam=camera, chip=''
;         endif

;         if instep eq 'split' and (lrisversion gt 1 or cameras[icam] eq 'blue') then begin  ; and n_elements(chips) eq 2 
;            if n_elements(chips) eq 1 then ch = chips[0] else ch = ''
;            autolrissplit, cam=camera, mode=mo, ch=ch
;         endif
...
;         if mo eq 'sp' then begin ;  not really necessary
;            if instep eq 'fluxcal' then autolrisfluxcal, chip='r', cam=camera ; could move this in the chip loop
;            if instep eq 'combine' then autolriscombine, camera=camera
;         endif
...
;   ; Camera has ceased to be relevant for the following
;
;   for imode = 0, nmodes-1 do begin
;
;      if mo eq 'sp' and instep eq 'connect' then autolrisconnect
;
;      ; a redblue imaging alignment procedure would also go here      
;
;   endfor
...
if nflatfail gt 1 then begin ;element 0 is blank
  ...
  for f = 1, n_elements(flatfail)-1 do begin
   ...
   ;, clip(sxpar(h,'WINDOW')), ' ', clip(sxpar(h,'PANE'))
  endfor
...
if nafail ge 1 then begin
  ...
  ;print, 'Finished with no astrometric problems.'
;endelse
...
; if file_test('match*') gt 0 then spawn, 'rm -f match*'
