...
pro autolrisdefaults
   ...
   for p = -1, 1 do begin
      ...
      sfile = path+'/lrisautoproc_opt.par'       ; get the configuration file
      n = countfiles(sfile)
...
pro lrisautoproc_opt, datadirectory=datadirectory, modestr=modestr, camerastr=camerastr, chipstr=chipstr, files=files, filters=filters, gratings=gratings, targets=targets, start=start, stop=stop, only=only, step=step, nocrclean=nocrclean, redo=redo, nofringe=nofringe, continuous=continuous
...
if lastdir eq 'imredux_opt/' or lastdir eq 'spredux/' then begin
